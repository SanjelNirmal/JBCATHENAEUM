    ...
    <section className="py-32">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-20">
        <div className="max-w-xl text-center md:text-left">
          ...
          <h2 className="text-3xl md:text-6xl font-serif font-bold text-[#002147] mb-6 tracking-tight">Focus Areas</h2>
          <p className="text-slate-500 font-light text-base md:text-xl leading-relaxed">Explore our most distinguished collections, meticulously indexed for serious academic research.</p>
        </div>
        <button 
          onClick={onViewAll}
        ...
        >
          <span className="text-xs font-black text-[#002147] group-hover:text-white uppercase tracking-[0.2em] transition-colors">Full Directory</span>
          <ArrowRight size={20} className="text-[#c49b63] group-hover:translate-x-1 transition-all" />
        </button>
      </div>
        ...
        {collections.map((item, index) => (
          ...
          >
            ...
            <div className="relative h-full bg-white border border-slate-100 rounded-[2.5rem] p-10 shadow-[0_12px_40px_-12px_rgba(0,0,0,0.03)] hover:shadow-[0_40px_80px_-20px_rgba(196,155,99,0.15)] hover:-translate-y-3 transition-all duration-700 flex flex-col items-start overflow-hidden group">
              ...
              <div className={`p-5 rounded-2xl ${item.color} border mb-10 relative z-10 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500`}>
                {item.icon}
              </div>
              
              <h3 className="text-2xl font-bold text-[#002147] mb-3 relative z-10 group-hover:text-[#c49b63] transition-colors duration-300 tracking-tight">{item.name}</h3>
              <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.3em] mb-12 relative z-10">Archive Index No. 0{index + 1}</p>
