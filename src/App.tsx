...
export default function App() {
  ...
  const [subjectId, setSubjectId] = useState<string>("System Architecture");
  const [infoPage, setInfoPage] = useState<string>('Support');
  const [user, setUser] = useState<UserData>(null);
  const [showLogin, setShowLogin] = useState<boolean>(false);

  const { subjects, getSubjectByName, loading } = useResourcesData();
  ...
  const currentSubjectData = getSubjectByName(subjectId) || {
    id: 'placeholder',
    faculty: '-',
    semester: '-',
    name: subjectId,
      ...
      {view === 'home' && (
        <>
          ...
          <main className="flex-1 max-w-7xl w-full mx-auto px-4 md:px-12 pb-24">
            <FeatureSplit onNavigateViewer={() => handleSelectSubject('System Architecture')} />
          </main>
        </>
      )}
