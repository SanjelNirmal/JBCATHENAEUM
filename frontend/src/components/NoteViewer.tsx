    ...
    <div className="flex flex-col md:flex-row w-full max-w-7xl mx-auto px-4 md:px-12 py-12 gap-8 font-sans animate-in zoom-in-95 duration-500">
      ...
      <aside className="w-full md:w-1/4 flex flex-col md:border-r border-slate-200 md:pr-8">
        ...
        <div className="space-y-4">
          <h4 className="text-xs uppercase tracking-widest text-slate-400 font-semibold mb-4">Other Notes in this Subject</h4>
          
          {subjectData.notes.length === 0 ? (
            <p className="text-sm text-slate-500 italic">No notes uploaded yet.</p>
          ) : (
            subjectData.notes.map((note) => (
              ...
              >
                <div className="flex justify-between items-start gap-2">
                  <h5 className={`text-sm font-semibold mb-1 ${selectedNote?.id === note.id ? 'text-[#002147]' : 'text-slate-800 group-hover:text-[#002147]'}`}>
                    {note.title}
                  </h5>
                  {selectedNote?.id === note.id && (
                     <span className="flex-shrink-0 text-[9px] uppercase tracking-widest font-bold bg-[#002147] text-white px-1.5 py-0.5 rounded-sm">Viewing</span>
                  )}
                </div>
                <p className="text-xs text-slate-500">By {note.author} &bull; {note.size}</p>
              </div>
            ))
          )}
        </div>
      ...
      <main className="w-full md:w-3/4 flex flex-col">
        {selectedNote ? (
            ...
            <div className="mb-8 border-b border-slate-200 pb-6">
              ...
              <div className="flex flex-wrap items-center gap-6 text-sm text-slate-500">
                <div className="flex items-center space-x-2">
                  <User size={16} />
                  <span>Uploaded by <span className="font-medium text-slate-800">{selectedNote.author}</span></span>
                </div>
                <div className="flex items-center space-x-2">
                  <Calendar size={16} />
                ...
                <div className="flex flex-1 justify-end space-x-3">
                   ...
                   <a 
                     href={selectedNote.url.replace('/preview', '/view')} 
                     target="_blank" 
            ...
            <div className="w-full h-[600px] md:h-[800px] border border-slate-200 rounded overflow-hidden relative shadow-sm">
                <iframe 
                 ...
                 <div className="absolute inset-0 z-0 bg-slate-50 flex flex-col justify-center items-center text-slate-400 p-8 text-center pointer-events-none">
                     <p className="text-sm font-semibold uppercase tracking-widest mb-2 text-[#002147]/40">Document Viewer</p>
          ...
          <div className="w-full flex-1 min-h-[60vh] flex flex-col items-center justify-center text-center p-12 bg-white border border-slate-200 rounded">
            <h2 className="text-2xl font-serif text-[#002147] mb-2">No notes available</h2>
            <p className="text-slate-500">There are currently no notes uploaded for this subject.</p>
          </div>
        )}
      </main>
    </div>
