...
import { CheckCircle2, CircleAlert, CircleHelp, Database, HardDrive, ShieldCheck } from "lucide-react";
...
const initialStatus: RuntimeStatus = {
  ...
  migrationDetail: "Checking the Stage 1 schema marker…",
  storageDetail: "Checking storage configuration…",
};

function StatusIcon({ state }: { state: CheckState }) {
  if (state === "healthy") return <CheckCircle2 className="text-emerald-600" size={20} />;
  if (state === "warning") return <CircleAlert className="text-amber-600" size={20} />;
  if (state === "unavailable") return <CircleAlert className="text-red-600" size={20} />;
...
export function SystemStatusPanel() {
  ...
  useEffect(() => {
    ...
    async function checkRuntime() {
      const [databaseResult, migrationResult] = await Promise.all([
        ...
        supabase.from("resource_submissions").select("id", { count: "exact", head: true }),
      ]);

      let storageState: CheckState = "warning";
      let storageDetail = "No VITE_RESOURCE_STORAGE_BUCKET is configured; uploads remain out of scope for Stage 1.";
      if (storageBucket) {
        const storageResult = await supabase.storage.from(storageBucket).list("", { limit: 1 });
  ...
  return (
    <section className="space-y-6">
      <div>
        <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#c49b63]">Read-only diagnostics</p>
        <h2 className="mt-2 text-2xl font-serif font-bold text-[#002147]">System status</h2>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
          This page cannot execute SQL or change infrastructure. It reports safe configuration metadata and live connectivity checks only.
        </p>
      </div>

      <div className="grid gap-4 lg:grid-cols-2">
        <StatusRow icon={<Database size={20} />} title="Supabase database" state={status.database} detail={status.databaseDetail} />
        <StatusRow icon={<ShieldCheck size={20} />} title="Latest database migration" state={status.migration} detail={status.migrationDetail} />
        <StatusRow icon={<HardDrive size={20} />} title="Supabase Storage" state={status.storage} detail={status.storageDetail} />
        <StatusRow
