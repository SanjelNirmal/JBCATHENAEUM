...
function isQuiet(preference: Record<string, unknown>): boolean {
  if (
  ...
  try {
    const timezone =
      typeof preference.timezone === "string" ? preference.timezone : "UTC";
...
Deno.serve(async (request: Request): Promise<Response> => {
  ...
  try {
    ...
    const eligible = subscriptions.filter((row) => {
      ...
      if (!preference || preference.push_enabled !== true) {
        return false;
      }
      ...
      const categoryAllowed =
        input.category === "administrative_announcement"
          ? preference.system_announcements
          : input.category === "past_question"
            ? preference.past_questions
            : input.category === "account_security"
              ? preference.account_security
              : input.category === "new_resource"
                ? preference.new_resources
                : preference.resource_updates;

      if (categoryAllowed !== true) {
        return false;
      }
    ...
    await runBounded(eligible, 8, async (subscription) => {
      ...
      const message = {
        ...
        webpush: {
          ...
          fcm_options: {
            link: `https://jbc.nirmalsanjel.com.np${targetUrl}`,
          },
        },
      ...
      const invalidToken =
        ["UNREGISTERED", "INVALID_ARGUMENT", "NOT_FOUND"].includes(
          detail.code,
        ) || /registration token|unregistered/i.test(detail.message);
    ...
    return jsonResponse(
      request,
      {
        ...
        // Temporarily useful for debugging.
        // Remove this field after the function works.
        details,
      },
      status,
    );
